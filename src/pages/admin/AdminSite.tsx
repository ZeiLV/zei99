...
import { applyAccent, type FooterLink } from "@/hooks/useSiteSettings";
...
const AdminSite = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!data) return;
      setAccent(data.accent_hsl);
      setFooterEnabled(data.footer_enabled);
      setFooterText(data.footer_text);
      setLinks(Array.isArray(data.footer_links) ? (data.footer_links as unknown as FooterLink[]) : []);
    })();
  }, []);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">Rang va footer sozlamalari</p>
      </div>
